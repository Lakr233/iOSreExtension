...
DUMP_JS = os.path.join(script_dir, 'dump_oem.js')

# ./dump_oem.py UDIDXXXXX TARGET.BUNDLE.ID ROOTPASSWORD PORT IPALOCATION
if (len(sys.argv) != 6):
    print("-> this script is OEM only")
    exit(-1)
...
# ./dump_oem.py UDIDXXXXX TARGET.BUNDLE.ID ROOTPASSWORD PORT IPALOCATION
name_or_bundleid = sys.argv[2]
output_ipa = sys.argv[5]
